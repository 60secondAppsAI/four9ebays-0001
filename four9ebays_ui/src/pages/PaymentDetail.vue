					...
					<tr> 
						<td style="width: 100%;"></td>
				        	<td style="width: 150px;">
				      			<base-button class="btn btn-primary" @click="updatePayment()">Save</base-button>
				        	</td>
				        	<td style="width: 150px;">
				        	<td style="width: 150px;">
								...
								<div >
								  <div class="container">
												    <base-button class="btn btn-primary" @click="modal2Payments = true">Add</base-button>
								              <modal :show.sync="modal2Payments">
          ...
          <a-modal :footer="null" :visible="userPermissionDialog" :closable="false" class="centered-modal" v-if="userPermissionDialog">
            <div style="text-align: center; padding: 20px;">
              ...
              <button @click="handlePermissionDialogClose">OK</button>
										...
										<tr> 
					<td class="detail_view_column_2">Payment Id</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="PaymentId" type="text" placeholder="Enter PaymentId" v-model="paymentDetails.paymentId"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Payment Amount</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<input label="PaymentAmount" type="text" placeholder="Enter PaymentAmount" v-model="paymentDetails.paymentAmount"></input>
																		</div>
						</td>
														</tr>
															<tr> 
					<td class="detail_view_column_2">Payment Date</td>
										    <td class="detail_view_column_1">
							<div class="detail_view_input" style="display: inline-flex;">
																		<datepicker v-model="paymentDetails.paymentDate"  placeholder="Select Date" ></datepicker>
																		</div>
						</td>
														</tr>
...
export default {
  ...
  methods: {
    ...
    async updatePayment() {
      ...
      const jsonData = JSON.stringify(this.paymentDetails);
      const res = await PaymentService.update(jsonData);
...
//        this.getAllPayments();
    ...
    async getPaymentDetails() {
      try {
			let response = await PaymentService.get(this.paymentId);
